...
class VotercheckController extends Controller
{
   ...
   public function datafromdb($id)
   {
    ...
    public function votercheck($id)
    {
            ...
            if(count($voter)==0 || $voter->vote_status!=0) 
      ...
      /*-----------------------------------------------------------------------*/
       ...
       return view::make('running')->with('data',$data)->with('post_count',$post_count)->with('post_list',$all_post)->with('candidate_list',$all_candidate)->with('voter_data',$voter);
